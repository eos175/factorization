...
struct wideint
{
    ...
    inline wideint(const udhlimb_t n)
        : limbs{0}
    {
        std::memcpy(limbs.data(), &n, sizeof(n));
    }
    ...
    /*! string */
    inline wideint(const std::string &str)
        : limbs{0}
    {
    ...
    inline wideint& operator=(const udhlimb_t n)
    {
        uint8_t *d = (uint8_t*)limbs.data();
        
        std::memcpy(d, &n, sizeof(n));
        std::memset(d + sizeof(n), 0, sizeof(limbs) - sizeof(n));
        return *this;
    }
    ...
    { return std::equal(limbs.begin(), limbs.end(), operand.limbs.begin()); }
    ...
    bool op_lte(const wideint &operand) const { return !(*this > operand); }
    bool op_gt(const wideint &operand) const { return operand < *this; }
    bool op_gte(const wideint &operand) const { return !(*this < operand); }
    ...
    wideint& operator++() { return op_inc(); }
    wideint& operator--() { return op_dec(); }
    wideint operator++(int) { value_type tmp(*this); operator++(); return tmp; }
    wideint operator--(int) { value_type tmp(*this); operator--(); return tmp; }
    ...
    explicit operator bool() const { return this->op_nez(); }
    ...
    {
        T tmp(0);
        size_t sz_min = std::min(sizeof(tmp), sizeof(limbs));
        std::memcpy(&tmp, limbs.data(), sz_min);
        return tmp;
    }
...
std::ostream& operator<<(std::ostream &os, const __int128_t &o) 
{ return os << wideint<128, true>(o).to_string(); }

std::ostream& operator<<(std::ostream &os, const __uint128_t &o) 
{ return os << wideint<128, false>(o).to_string(); }
#endif


template <size_t bits, bool is_signed = true, size_t limb_bits_param = 64 >
std::ostream& operator<<(std::ostream &os, const wideint<bits,is_signed,limb_bits_param> &o) 
